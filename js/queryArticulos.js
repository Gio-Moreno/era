document.addEventListener("DOMContentLoaded", function() {
    ...
    fetch(jsonFile)
        ...
        .then(data => {
            ...
            const article = data.find(article => article.idpost === parseInt(idpost) && article.nivel === nivel);
            if (article) {
                ...
                const articleHTML = `
                    <section class="bg-dark">
                        <div class="container text-center py-5">
                            <div id="imagen"></div>
                            <p class="text-uppercase"><strong></strong>${article.categorias}</p>
                            <h2 class="text-light m-0 fs-1"><span class="text-primary text-uppercase">${article.titulo}</span></h2>
                            <p><strong>Autor:</strong> ${article.autor.nombre}</p>
                        </div>
                    </section>
                            ...
                            <div class="container">
                                <div class="g-5">
                                    <div class="wow" data-wow-delay="0.5s">
                                        <div class="h-100">
                                            <h4 class="mb-3" id="">${article.extracto}</h4>
                                            <p class="fs-5">${article.contenido}</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
